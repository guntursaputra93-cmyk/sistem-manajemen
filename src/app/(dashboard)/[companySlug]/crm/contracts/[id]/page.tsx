...
export default async function ContractDetailPage({
  ...
  return (
    <div className="max-w-2xl space-y-8">
      <div>
        <Link href={`/${companySlug}/crm/contracts`} className="text-sm text-blue-600 hover:underline">&larr; Kembali</Link>
        <h1 className="text-xl font-bold text-gray-900 mt-2">{opp?.title ?? "Contract"}</h1>
      ...
      <section className="bg-white border border-gray-100 rounded-xl p-6">
        <h2 className="font-semibold text-gray-900 mb-4">Detail Contract</h2>
        {canManage ? (
          <form action={updateContractAction} className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Nilai Kontrak (Rp)</label>
              <input name="contractValue" type="number" step="0.01" defaultValue={contract.contractValue} required className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Status Pembayaran</label>
              <select name="paymentStatus" defaultValue={contract.paymentStatus} required className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
            ...
            <div>
              ...
              <input name="startDate" type="date" defaultValue={contract.startDate} required className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Tanggal Selesai</label>
              <input name="endDate" type="date" defaultValue={contract.endDate ?? ""} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div className="col-span-2">
              <label className="block text-xs font-medium text-gray-700 mb-1">Tanggal Reminder Renewal (opsional)</label>
              <input name="renewalReminderDate" type="date" defaultValue={contract.renewalReminderDate ?? ""} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div className="col-span-2">
          ...
          <dl className="text-sm space-y-2">
            <div><dt className="text-gray-500 inline">Nilai Kontrak: </dt><dd className="inline">Rp {Number(contract.contractValue).toLocaleString("id-ID")}</dd></div>
            <div><dt className="text-gray-500 inline">Status Pembayaran: </dt><dd className="inline">{PAYMENT_STATUS_LABEL[contract.paymentStatus]}</dd></div>
            <div><dt className="text-gray-500 inline">Mulai: </dt><dd className="inline">{contract.startDate}</dd></div>
            <div><dt className="text-gray-500 inline">Selesai: </dt><dd className="inline">{contract.endDate ?? "-"}</dd></div>
            <div><dt className="text-gray-500 inline">Reminder Renewal: </dt><dd className="inline">{contract.renewalReminderDate ?? "-"}</dd></div>
          </dl>
        )}
      </section>
    </div>
